...
bool InitializeTexture(MyTexture *texture, const string &imageFileName)
{
    ...
    // bind the 2D texture to access using u,v coordinates in range [0, 1]
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, texture->width, texture->height, 0,
      GL_RGBA, channelDataType, 0);
#else
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, texture->width, texture->height, 0,
      GL_BGRA, channelDataType, 0);
#endif
...
#if defined(_WIN32)
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, row, texture->width, 1, GL_RGBA,
          channelDataType, pixels + offset);
#else
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, row, texture->width, 1, GL_BGRA,
                        channelDataType, pixels + offset);
#endif
    }
